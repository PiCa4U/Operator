...
function extractSipHost(wsServer: string): string {
    try {
        const url = (wsServer.startsWith('ws://') || wsServer.startsWith('wss://'))
            ? new URL(wsServer) : new URL(`wss://${wsServer}`);
        return url.port ? `${url.hostname}:${url.port}` : url.hostname;
    } catch { return '24webrtc.ru'; }
}
...
// общий обработчик 403 при REGISTER
function onRegisterRejectMaybeRefreshHa1(res: any) {
    const code = Number(res?.message?.statusCode ?? res?.statusCode ?? 0);
    if (code === 403) {
...
export function useSipUA(config: {
    ...
    // ToneManager + внешний конфиг
    const tonesRef  = useRef<ToneManager | null>(null);
    const extCfgRef = useRef<AppExternalConfig>(readExternalConfig());

    useEffect(() => {
        // дефолты: локальные файлы из public/tones
        const buildSources = (cfg: AppExternalConfig) => {
            const base = ((cfg.assetsBase ?? '/').replace(/\/?$/, '/')); // гарантируем trailing slash
            const t = cfg.tones || {};
            return {
                ringback: t.ringback ?? `${base}tones/ringback.mp3`,
                busy:     t.busy     ?? `${base}tones/busy.mp3`,
                reorder:  t.reorder  ?? `${base}tones/reorder.mp3`,
                // у тебя файл называется iphone-11-pro.mp3 — используем его по умолчанию
                incoming: t.incoming ?? `${base}tones/iphone-11-pro.mp3`,
            };
        };
    ...
    function bind(s: Session) {
        ...
        webrtcOwner.setBusy(true);

        (s.delegate ??= {}).onBye = () => { playEndToneOnce(1500); };
        ...
        s.stateChange.addListener(st => {
            ...
            if (st === SessionState.Terminated) {
                webrtcOwner.setBusy(false);

                const remaining = endToneUntilRef.current - Date.now();
                if (remaining > 0) setTimeout(() => tonesRef.current?.stopAll(), remaining);
                else tonesRef.current?.stopAll();
    ...
    async function clearUA(reason: ClearReason = 'shutdown') {
        ...
        try {
            if (reason === 'shutdown' && wasStartedRef.current && !sentDeleteRef.current && sessionKey && webrtcOwner.isOwner()) {
    ...
    async function initUA(creds: TurnCredentials | null) {
        ...
        try {
            ...
            const uaOptions: UserAgentOptions = {
                ...
                sessionDescriptionHandlerFactoryOptions: {
                    ...
                    peerConnectionConfiguration: {
                        iceServers: [{ urls: 'stun:stun.l.google.com:19302' }, creds || undefined].filter(Boolean) as RTCIceServer[],
                    },
            ...
            const doRegister = () =>
                registerer.register({ requestDelegate: { onReject: onRegisterRejectMaybeRefreshHa1 } });
    ...
    useEffect(() => {
        if (!enabled) return;
        if (!turnCreds) return;
        const same = JSON.stringify(latestTurnCredsRef.current) === JSON.stringify(turnCreds || null);
        if (same) return;
        latestTurnCredsRef.current = turnCreds;
    ...
    useEffect(() => {
        ...
        if (ua && reg && reg.state !== RegistererState.Terminated) {
            ...
            reg.register({ requestDelegate: { onReject: onRegisterRejectMaybeRefreshHa1 } }).catch(() => {});
        }
    }, [enabled, ha1]);
    ...
    const makeCall = async (target: string) => {
        ...
        const sipHost = extractSipHost(wsServer);
        const targetUri = UserAgent.makeURI(target?.startsWith('sip:') ? target : `sip:${target}@${sipHost}`) as URI;
            ...
            requestDelegate: {
                ...
                onReject: (response: SipResponseLite) => {
                    ...
                    if ([486,600,603].includes(code)) {
                        tonesRef.current?.play('busy'); setTimeout(() => tonesRef.current?.stopAll(), 2500);
                    } else if ([500,503,480,408].includes(code)) {
                        tonesRef.current?.play('reorder'); setTimeout(() => tonesRef.current?.stopAll(), 2500);
                    }
                },
            },
        });
